...
const inspectPointer = async (page, x, y) => {
  return page.evaluate(({ x, y }) => {
    const hit = document.elementFromPoint(x, y);
    const overlay = document.querySelector('.tbl-overlay-container');
    const shiftGuide = document.querySelector('.tbl-shift-guide');
    const selectedTable = document.querySelector('.sc-editor-root table.sc-selected-table');
    return {
      hitTag: hit?.tagName ?? null,
      hitClass: hit?.className ?? null,
      hitText: (hit?.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 60),
      shiftGuideClass: shiftGuide?.className ?? null,
      shiftGuideStyle: shiftGuide?.getAttribute('style') ?? null,
      overlayStyle: overlay?.getAttribute('style') ?? null,
      selectedTableClass: selectedTable?.className ?? null,
      overlayCount: document.querySelectorAll('.tbl-overlay-container').length,
    };
  }, { x, y });
};

const debugHoverShiftBoundary = async (page, tableIndex, rowIndex, cellIndex, edge) => {
  ...
  for (const probe of probes) {
    ...
    await sleep(120);
    const inspected = await inspectPointer(page, x, y);
    console.log(`probe=${probe}`, JSON.stringify(inspected, null, 2));

    const guide = await page.$('.tbl-shift-guide');
    if (guide) {
      console.log('guide-found', { x, y });
      return { x, y };
    }
  }
...
const main = async () => {
  ...
  try {
    ...
    const rowTableIndex = await getLastTableIndex(page);
    await debugHoverShiftBoundary(page, rowTableIndex, 0, 0, 'bottom');
    await dragShiftGuide(page, 0, 22);
    await debugHoverShiftBoundary(page, rowTableIndex, 0, 1, 'bottom');
    await dragShiftGuide(page, 0, 16);
    console.log('row-after-snap', JSON.stringify(await getTableDebug(page, rowTableIndex), null, 2));
    await debugHoverShiftBoundary(page, rowTableIndex, 0, 1, 'bottom');
    await dragShiftGuide(page, 0, -14);
    console.log('row-after-neighbor-up', JSON.stringify(await getTableDebug(page, rowTableIndex), null, 2));

    await insertThreeByThreeTable(page);
    const freshTableIndex = await getLastTableIndex(page);
    console.log('fresh-table-index', freshTableIndex);
    await debugHoverShiftBoundary(page, freshTableIndex, 0, 0, 'bottom');

    await insertThreeByThreeTable(page);
    const colTableIndex = await getLastTableIndex(page);
    console.log('col-table-index', colTableIndex);
    await debugHoverShiftBoundary(page, colTableIndex, 0, 0, 'right');
    await dragShiftGuide(page, 24, 0);
    console.log('col-after-first-right', JSON.stringify(await getTableDebug(page, colTableIndex), null, 2));
    await debugHoverShiftBoundary(page, colTableIndex, 1, 0, 'right');
    await dragShiftGuide(page, 16, 0);
    console.log('col-after-snap', JSON.stringify(await getTableDebug(page, colTableIndex), null, 2));
  } finally {
    await browser.close();
  }
};
