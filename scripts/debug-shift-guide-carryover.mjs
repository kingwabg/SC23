...
const inspectState = async (page, label) => {
  const state = await page.evaluate(marker => {
    const tables = Array.from(document.querySelectorAll('.sc-editor-root table'));
    const selectedIndex = tables.findIndex(table => table.classList.contains('sc-selected-table'));
...
const hoverShiftBoundary = async (page, tableIndex, rowIndex, cellIndex, edge) => {
  ...
  await sleep(120);
  await inspectState(page, `after-click-${tableIndex}-${rowIndex}-${cellIndex}`);
  await page.keyboard.down('Shift');
  await inspectState(page, `after-shift-down-${tableIndex}-${rowIndex}-${cellIndex}`);
  ...
  for (const probe of probes) {
    ...
    await sleep(150);
    const probeState = await page.evaluate(({ x, y, expectedAxis }) => {
      const hit = document.elementFromPoint(x, y);
      const guide = document.querySelector(`.tbl-shift-guide.${expectedAxis}`);
      return {
    ...
    console.log(`probe-${probe}`, JSON.stringify(probeState, null, 2));

    if (probeState.guideFound) {
      return { x, y };
    }
  }
...
const main = async () => {
  ...
  try {
    ...
    const firstTable = await getLastTableIndex(page);
    await hoverShiftBoundary(page, firstTable, 0, 0, 'bottom');
    await dragShiftGuide(page, 0, 22);
    await hoverShiftBoundary(page, firstTable, 0, 1, 'bottom');
    await dragShiftGuide(page, 0, 16);
    await inspectState(page, 'after-first-table-two-shifts');

    await insertThreeByThreeTable(page);
    const secondTable = await getLastTableIndex(page);
    await inspectState(page, 'after-second-table-insert');
    await hoverShiftBoundary(page, secondTable, 0, 0, 'bottom');
    console.log('second-table-first-guide-found');
  } finally {
    await browser.close();
  }
};
